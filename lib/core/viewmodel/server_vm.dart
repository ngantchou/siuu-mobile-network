...
import 'package:Siuu/nearby/roomtalk.dart';
import 'package:flutter/material.dart';
import 'package:get_ip/get_ip.dart';
import 'package:Siuu/core/model/message.dart';
import 'package:Siuu/core/model/tcpData.dart';
import 'package:Siuu/database/roomtalk_dao.dart';
...
class ServerViewModel extends ChangeNotifier {
  ...
  void startServer(
      context, String ipAdress, String portNumber, String userName) async {
    RoomTalkDao roomTalkDao = new RoomTalkDao();
    ...
    try {
      ...
      name.text = userName;
      _server =
          await ServerSocket.bind(ip.text, int.parse(port.text), shared: true);
      //notifyListeners();
  ...
  void sendMessage(context, TCPData tcpData, String type,
      {bool isHost, String messages}) async {
    RoomTalkDao roomTalkDao = new RoomTalkDao();
    String senderIp = "";
    if (!Platform.isMacOS) senderIp = await GetIp.ipAddress;
    print(senderIp);
    var message = utf8.encode(json.encode(Message(
            message: messages != null ? messages : msg.text,
            name: tcpData?.name ?? '',
            ip: senderIp,
            type: type,
