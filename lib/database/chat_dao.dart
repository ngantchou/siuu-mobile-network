...
import 'package:Siuu/database/app_database.dart';
...
class ChatDao {
  ...
  Future delete(String chatId) async {
    final finder =
        Finder(filter: Filter.and([Filter.equals("chatId", chatId)]));
  ...
  Future<List<Chat>> findChat(String chatId) async {
    ...
    final recordSnapshots = await _chatStore.find(await _db, finder: finder);
    return recordSnapshots.map((snapshot) {
      final fruit = Chat.fromJson(snapshot.value);
      return fruit;
    }).toList();
